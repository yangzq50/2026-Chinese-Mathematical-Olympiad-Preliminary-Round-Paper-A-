...
第三题采用首尾配对与余量补偿：先证每对的和至多为 2，
再用中间四对各自节省的余量，补足首项超过 1 的部分。

第四题的偶数情形用两次计数得到关于全表总和与中间两行总和的两个不等式，
消元即得下界；取等填法统一用一个两列整数块给出。正文保留必要的错位铺放示意图。
...
`check_s_shapes.py` 核验第四题的小规模线性规划最优值、取等构造及双计数覆盖次数，可以用以下命令运行：
